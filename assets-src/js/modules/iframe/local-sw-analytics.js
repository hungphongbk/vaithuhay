  ...
  return self.registration.pushManager
    ...
    .then(response => {
      if (!response.ok) {
        ...
        console.log(
          `${eventCategory}/${eventAction}hit sent, check the Analytics dashboard. Label=${label}`
        );
      }
    })
    .catch(err => {
...
export default function sendPushNotificationAnalyticsEvent(
  eventAction,
  eventCategory,
  notificationId = null
) {
  if (notificationId) notificationId = "noti/" + notificationId;
