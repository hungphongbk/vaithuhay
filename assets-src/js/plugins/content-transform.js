  ...
  $html.children().each((_, child) => {
    ...
    if ($child) {
      ...
      switch ($child.nodeName) {
        case "BR":
          if ($(child).children.length === 1) $(child).detach();
          break;
        case "IMG":
          $(child).data("type", "img");
    ...
    $html.children().map(
      (_, child) =>
        new Promise(async resolve => {
          if (
            $(child).data("type") === "vaithuhay" &&
            $(child).data("embed-type") === "wholesale"
          ) {
            // console.log($(child).data('embed-data'))
            const { apply, rule: uuid } = $(child).data("embed-data"),
              [{ rules }] = await $.get(
                `https://server.vaithuhay.com/b/products/${apply}/wholesale?i=${uuid}`
              );
            // console.log(rules);
            $(child).replaceWith(
              `<product-whole-sale-item class='in-content' :apply='${apply *
                1}' :rule=\'${JSON.stringify(
                rules[0]
              )}\'></product-whole-sale-item>`
            );
          }
          resolve();
        })
  ...
  function process(stack) {
    ...
    stack.forEach(($el, i) => {
      ...
      if (i !== count - 1) $el.detach();
        ...
        $el.replaceWith(
          `<product-images :images_=\'${JSON.stringify(
            images.map(i => ({
              small: i,
              original: i
            }))
...
export default async (transforms, component) => {
  ...
  for (const t of transforms) {
    if (typeof t === "string") await list[t].call(component, $content, {});
    else {
      const [f, opt] = t;
