...
const SystemModal = {
  ...
  render(h, {data}) {
    ...
    const renderedCommands = commands.map(({label, callback, isPrimary = false}) => (
      <div class={['btn', isPrimary ? 'btn-theme-red' : 'btn-default']} onClick={callback}>{label}</div>));

    return (<Modal title={title} is-dismissable={isDismissable}>
...
export default (title, content, commands, _options = {}) => new Promise(resolve => {
  ...
  $modalEvent.$emit(SYSTEM_MODAL_SHOW, {title, content, commands: _commands, options: _options});
})
